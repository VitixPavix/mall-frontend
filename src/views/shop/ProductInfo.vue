<template>
  <div class="product-info-container">
    ...
    <div class="page-header">
      ...
      <p class="page-desc">管理您的商品信息，包括新增、编辑、删除等操作</p>
    </div>
...
const fetchProductList = async () => {
  ...
  try {
    const params = {
      ...
      createUserId: userInfoStore.info.id // 只获取当前店铺的商品
    }
    ...
    if (res.code === 0) {
      ...
      // 直接使用后端返回的数据，包含categoryName
      productList.value = rawData.map(item => ({
        ...item,
        shopname: '当前店铺' // 店铺自己的商品，显示当前店铺
      }))
    }
  } catch (error) {
...
const handleEdit = (row) => {
  ...
  // 设置详细图片列表 - detailImg 和 detailHtml 是独立的数据源
  detailImages.value = []
  ...
  console.log('detailImages.value:', detailImages.value)
  editDialogVisible.value = true
}
...
const handleSave = async () => {
  ...
  try {
    ...
    const productData = {
      ...
      price: editForm.price.toString(), // 确保价格是字符串
      stock: editForm.stock.toString(), // 确保库存是字符串
      detailImg: detailImages.value.length > 0 ? detailImages.value : null, // 详细图片数组，独立存储
      detailHtml: editForm.detailHtml || '', // 富文本内容，独立存储
    ...
    const apiCall = isEdit ? updateProductService : addProductService
    
    const res = await apiCall(productData)
    if (res.code === 0) {
