<template>
  <div class="user-management-container">
    ...
    <div class="page-header">
      ...
      <p class="page-desc">管理平台所有普通用户，包括查看、编辑、删除等操作</p>
    </div>
    ...
    <div class="table-section">
      ...
      >
        ...
        <el-table-column prop="balance" label="余额" width="100">
          <template #default="{ row }">
            <span class="balance-text">¥{{ row.balance || 0 }}</span>
          </template>
        </el-table-column>
...
const fetchUserList = async () => {
  ...
  try {
    ...
    const res = await getUserListService(requestData)
    if (res.code === 0) {
      let rawData = []
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      const res = await deleteUserService(selectedIds.value)
      if (res.code === 0) {
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEdit) {
      userData.id = editForm.id
      res = await updateUserService(userData)
    } else {
      // 新增用户时，后端会自动设置默认密码123456
      res = await addUserService(userData)
    }
...
const handleConfirmResetPassword = async () => {
  ...
  try {
    ...
    const res = await userResetPasswordService(resetData)
    if (res.code === 0) {
