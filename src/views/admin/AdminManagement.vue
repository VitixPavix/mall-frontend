<template>
  <div class="admin-management-container">
    ...
    <div class="page-header">
      ...
      <p class="page-desc">管理平台所有管理员账户，包括查看、编辑、重置密码等操作</p>
    </div>
      ...
      >
        ...
        <el-table-column label="角色" width="120">
          <template #default="{ row }">
            <el-tag 
              :type="row.id === 1 ? 'danger' : 'warning'"
              size="small"
            >
              {{ row.id === 1 ? '超级管理员' : '普通管理员' }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="{ row }">
            ...
            <el-button 
              ...
              :disabled="row.id === currentAdminId || row.id === 1"
            >
              <el-icon><Delete /></el-icon>
              删除
            </el-button>
      ...
      >
        ...
        <el-form-item label="新密码" prop="newPassword">
          <el-input 
            ...
            type="password" 
            placeholder="留空则重置为默认密码Admin@123456"
            show-password
            clearable
          />
        </el-form-item>
        ...
        <div class="reset-password-tip">
          <el-alert
            title="提示：如果不输入新密码，系统将自动重置为默认密码 Admin@123456"
            type="info"
            :closable="false"
            show-icon
          />
...
// 当前登录管理员ID
const currentAdminId = computed(() => userInfoStore.info.id)
...
const fetchAdminList = async () => {
  ...
  try {
    ...
    if (searchForm.nickname && searchForm.nickname.trim()) {
      requestData.nickname = searchForm.nickname.trim()
    }
    ...
    const res = await getAdminListService(requestData)
    if (res.code === 0) {
      let rawData = []
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    ...
    try {
      const res = await deleteAdminService(row.id)
      if (res.code === 0) {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(
    `确定要删除选中的 ${selectedIds.value.length} 个管理员吗？此操作不可恢复！`,
    '批量删除确认',
    {
      confirmButtonText: '确定删除',
      cancelButtonText: '取消',
    ...
    try {
      const res = await deleteAdminService(selectedIds.value)
      if (res.code === 0) {
        ElMessage.success(`成功删除 ${selectedIds.value.length} 个管理员`)
        selectedIds.value = []
        fetchAdminList()
      }
    } catch (error) {
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEdit) {
      adminData.id = editForm.id
      res = await updateAdminService(adminData)
    } else {
      // 新增管理员时，后端会自动设置默认密码Admin@123456
      res = await addAdminService(adminData)
    }
    
    if (res.code === 0) {
      ElMessage.success(isEdit ? '更新成功' : '新增成功，默认密码为Admin@123456')
...
const handleConfirmResetPassword = async () => {
  ...
  try {
    ...
    // 如果输入了新密码，则使用新密码；否则后端会使用默认密码Admin@123456
    ...
    const res = await adminResetPasswordService(resetData)
    if (res.code === 0) {
      const message = resetPasswordForm.newPassword 
        ? '密码重置成功' 
        : '密码重置成功，新密码为：Admin@123456'
