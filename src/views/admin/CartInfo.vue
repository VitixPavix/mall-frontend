<template>
  <div class="cart-info-container">
    ...
    <div class="page-header">
      ...
      <p class="page-desc">管理平台所有用户的购物车记录，包括查看、搜索、删除等操作</p>
    </div>
    ...
    <div class="search-section">
      <el-form :model="searchForm" inline class="search-form">
        <el-form-item label="店铺用户名">
          <el-input
            v-model="searchForm.shopUserName"
            placeholder="请输入店铺用户名"
            clearable
            style="width: 200px"
          />
        </el-form-item>
    ...
    <div class="table-section">
      ...
      >
        ...
        <el-table-column prop="createUserName" label="创建用户" width="120" />

        <el-table-column prop="shopUserName" label="店铺用户名" width="120" />
...
const fetchCartList = async () => {
  ...
  try {
    ...
    // 添加搜索条件
    if (searchForm.shopUserName && searchForm.shopUserName.trim()) {
      params.shopUserName = searchForm.shopUserName.trim()
    }
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    `确定要删除商品"${row.productName}"的购物车记录吗？此操作不可恢复！`,
    '删除确认',
    {
...
const handleBatchDelete = () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请选择要删除的购物车记录')
    return
  }

  ElMessageBox.confirm(
    `确定要删除选中的 ${selectedIds.value.length} 条购物车记录吗？此操作不可恢复！`,
    '批量删除确认',
    {
      confirmButtonText: '确定删除',
      cancelButtonText: '取消',
    ...
    try {
      ...
      if (res.code === 0) {
        ElMessage.success(`成功删除 ${selectedIds.value.length} 条购物车记录`)
        selectedIds.value = []
        fetchCartList()
      } else {
